...
router.post('/produccion', (req, res) => {
    ...
    //res.json(req.file)
    ffmpeg(path.join(__dirname, '../public/uploads/' + req.file.filename)).addOptions([
        /*'-c:v libx264',
        '-c:a aac',
        '-ac 1',
        '-strict -2',
        ...
        '-start_number 0',
        '-f hls'
    ]).output(path.join(__dirname, '../public/uploads/trasn/video_1080p.m3u8'))
        .on('end', function () {
            console.log("done")
        })
        .run()
    /*.on('error', function (err) {
        console.log('nose que es esto' + err.message);
    })
    .pipe(res, { end: true })*/
});
