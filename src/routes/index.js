...
//////* Quitar valores repetidos en un array *//////
Array.prototype.unique = function (a) {
    return function () { return this.filter(a) }
}(function (a, b, c) {
    return c.indexOf(a, b + 1) < 0
});
//////* Borrar valores undefined en un array *//////
...
router.get('/', async (req, res) => {
    ...
    contenido.map((f) => {
        generos = generos.concat(f.sesiones.split("-"))
        //return new Playerjs({ id: `player${f.id}` })
    })

    generos.unique().map((f) => {
        contnido = { genero: f }
        contnido.pelis = contenido.map((z) => {
            if (z.sesiones.split("-").includes(f)) {
                foto = z.imagenes.split(" - ")
                return {
                    poster: foto[1], trailer: z.ruta, id: z.id,
                    titulouno: z.titulouno, titulodos: z.titulodos,
                    trailer: z.trailer, ruta: z.ruta, fecha: z.fecha,
                    sesiones: z.sesiones, sinopsis: z.sinopsis,
                    slogan: z.slogan, poster2: foto[0]
                }
            }
        }).unique()
        datos.push(contnido)
    });
    const file = contenido.map((z) => {
        foto = z.imagenes.split(" - ")
        return { title: z.titulodos, imagen: foto[0], file: z.ruta, id: z.id, trailer: z.trailer }
    })
    res.render('index', { datos, file });
});


router.post('/', async (req, res) => {
    const contenido = await pool.query('SELECT * FROM contenidos');
    const file = contenido.map((z) => {
        foto = z.imagenes.split(" - ")
        return { title: z.titulodos, imagen: foto[0], file: z.ruta, id: z.id, trailer: z.trailer }
    })
